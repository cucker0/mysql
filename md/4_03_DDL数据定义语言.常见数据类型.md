...
* 定点型
    * decimal (M, D)
        >decimal也可以简写成 dec (M, D)，M范围：[1, 65]，D方位：[0, 30]
...
decimal(M,D)|(M + 2) 字节 |最大取值范围与double相同，<br>给定decimal的有效取值范围由M和D决定 |M范围：[1, 65]，D方位：[0, 30]
...
DATE | |java.sql.Date |91 |java.sql.Date |java.util.Date与DATE、LocalDateTime不能直接操作，[需要转化](https://github.com/cucker0/java_2019/blob/master/java8_features/src/com/java/time/timeConvert.java)，下面的类似，为简单java中也可以只用JDBC对应的类型
