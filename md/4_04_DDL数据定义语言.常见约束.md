...
:--- |:--- |:--- |:--- |:--- |:---
主键约束  |是          |否                  |最多1个  |是，不推荐，组合的每个字段不许为null,组合的值唯一 |自动建立唯一索引
唯一约束  |是          |无not null限制情况下，允许多个null值    |可有多个  |是，不推荐，表示多个字段组合后的值是唯一 |会自动创建一个同名的唯一索，该索引不能单独删除，删除约束会自动删除索引。唯一约束是通过唯一索引来实现数据的唯一。
