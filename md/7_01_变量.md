...
用户变量 |@变量名 |当前连接会话,<br>或在begin ... end代码块中  |自定义变量 |会话的任何地方 |set @变量名 = 值;  <br>或 set @变量名 := 值; <br>或 select @变量名 := 值; <br>set @num = 0, @course = '00'; <br>不用指定类型，必须赋初始值 |SET @变量名 = 新值; <br>SELECT 字段 INTO @变量名 FROM 表名; 
局部变量 |变量名 |只能在begin ... end代码块中 |自定义变量 |在begin ... end代码块中，begin ... end的首行，<br>不加@，需要指定类型 |DECLARE 局部变量名 INT DEFAULT 值;  <br>必须赋初始值 |SET 变量名 = 新值; <br>SELECT 字段 INTO 变量名 FROM 表名; 
