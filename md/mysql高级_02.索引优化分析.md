...
#### 两表join连接查询优化总结
1. 对于左外连接、右外连接查询，把连接条件的字段建为索引，且把索引建在从表上性能更好。为从表值只查询两表相同部分的行，主表查询所有行。  
   LEFT OUTER JOIN 的索引建在`右表`，RIGHT OUTER JOIN 的索引建在`左表`。
2. 对于内连接无此差别。
3. 用行数少结果集的表驱动大结果集的表，即在结果集行数少的表上建索引效果更好。
...
#### 三表join连接查询优化总结
1. 尽可能减少join语句中的嵌套循环总次数
2. 永远用小结果集 驱动 大结果集，即在小结果集表的字段中建索引
3. 优先优化嵌套循环的内层循环
4. 保证join语句中被驱动表上join条件字段已经被索引，被驱动的表为从表(结果集行数少的表)。  
    LEFT OUTER JOIN 的索引建在`右表`，RIGHT OUTER JOIN 的索引建在`左表`。
5. 当无法保证被驱动表的join条件字段被索引且内存资源充足的情况下，把my.cnf配置文件中 join_buffer_size 设置大点
...
### 索引优化小总结口诀
```text
全值匹配我最爱（SELECT 字段列表 与 索引字段匹配），最左前缀要遵守；
带头大哥不能死，中间兄弟不能断；  // 复合索引
索引列上少计算，范围之后全失效；  // 复合索引
like % 写最右，覆盖索引不写星（SELECT *）；
...
* 使用复合索引的时，索引字段顺序中越靠左的字段过滤性能越好
* 使用复合索引的时，尽量使用覆盖索引，即尽量都用索引中的字段作为查询字段
* 尽可能通过分析统计信息和调整query的方法来达到选择合适索引的目的
